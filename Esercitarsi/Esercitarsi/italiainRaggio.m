...
PuntoMedioX = mean(Punti(:,1));

%faccio il segmento
Seg = [PuntoMedioX,PuntoMinY;PuntoMedioX,PuntoMaxY];
bezSeg = curv2_bezier_interp(Seg,0,1,0);
[IP1P2,t1,t2]=curv2_intersect(ppbezCurvG,bezSeg);
[sx21,dx21]=ppbezier_subdiv(bezSeg,t2(1));
[sx22,bezSeg]=ppbezier_subdiv(sx21,t2(2));
%curv2_ppbezier_plot(bezSeg,60,'b',2);

%metà segmento di metà 
pMean = mean(bezSeg.cp(:,2));
f = (pMean*2)*3/100;
...
ppbezCurv.cp = point_trans(ppbezCurv.cp,S*T);
